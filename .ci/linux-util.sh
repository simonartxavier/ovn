...
# ovn_upgrade_build LOG_FILE
# Builds current version of OVN
function ovn_upgrade_build()
{
    local log_file=$1
    log "Building OVN"
    ./boot.sh >> "$log_file" 2>&1 &&
        ./configure CFLAGS="-O2 -g" >> "$log_file" 2>&1 &&
        make -j"$(nproc 2>/dev/null || echo 4)" >> "$log_file" 2>&1 || return 1
}

# ovs_ovn_upgrade_build LOG_FILE
# Builds current version of OVS and OVN
function ovs_ovn_upgrade_build()
{
    local log_file=$1
    log "Building OVS"
    (cd ovs && ./boot.sh > "$log_file" 2>&1 &&
         ./configure CFLAGS="-O2 -g" >> "$log_file" 2>&1 &&
         make -j"$(nproc 2>/dev/null || echo 4)" >> "$log_file" 2>&1 &&
         cd .. || return 1)
    ovn_upgrade_build $log_file || return 1
}
...
function ovn_upgrade_checkout_base()
{
    ...
    if git checkout "$base_version" >> "$log_file" 2>&1; then
        ...
        # Fall back to full fetch (for commit IDs or other refs)
        else
            log "Fetching all refs (may be a commit ID)..."
            git fetch origin >> "$log_file" 2>&1 || return 1
        fi

        # Now try checkout again
        if ! git checkout "$base_version" >> "$log_file" 2>&1; then
            log "ERROR: Failed to checkout $base_version" >&2
            return 1
        fi
    fi
...
# Some upgrade tests might fail as (new) ovn-controller added functionality
# such as new flows in a table which was checked in an old ovn-controller test.
# Add ability to skip that test.
function ovn_upgrade_get_skip_list()
{
    local base_version=$1
    local skip_list=""
    ...
    case "$base_version" in
        branch-25.03|v25.03.*)
            # Better prio for patch ports since "controller: Fix handling of
            # local/non-local routes to be advertised."
            skip_list="208|209|210|211"
            ;;

        branch-25.09|v25.09.*)
            # Table OFTABLE_LEARN_REMOTE_FDB added by "controller: Learn EVPN
            # workload FDB from the ingress traffic."
            skip_list="238|239"
            ;;
    esac
...
# adjust_test_range RANGE SKIP_LIST
# Returns adjusted test range
function ovn_upgrade_adjust_test_range()
{
    local range=$1
    local skip_list=$2
    ...
    range=$(make check-kernel TESTSUITEFLAGS="$range --list" |
            awk -F: '/^ *[0-9]+:/ {print $1}' |
            grep -vE "^ *(${skip_list}) *$")
    echo $range
}
