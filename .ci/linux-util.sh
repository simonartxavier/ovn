...
function ovn_upgrade_adjust_test_range()
{
    ...
    local output
    if ! output=$(make check-kernel TESTSUITEFLAGS="$range --list" 2>&1); then
        log "ERROR: Invalid test range '$range'" >&2
        return 1
    fi

    range=$(echo "$output" | awk -F: '/^ *[0-9]+:/ {print $1}' |
            grep -vE "^ *(${skip_list}) *$")

    if [ -z "$range" ]; then
        log "ERROR: No tests remaining after applying skip list to range '$1'" >&2
        return 1
    fi
